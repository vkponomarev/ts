...
class MonthsController extends Controller
{
    ...
    public function actionMonthPage($urlMonth)
    {
         ...
         * $urlCheckData['month']
         */
        $urlCheckData = $urlCheck->month($urlMonth);

        $main = new Main();
        Yii::$app->params['language'] = $main->language(Yii::$app->language);
        Yii::$app->params['language']['all'] = $main->languages();
        Yii::$app->params['text'] = $main->text($textID, Yii::$app->params['language']['current']['id']);
        Yii::$app->params['canonical'] = $main->Canonical($urlMonth, $mainUrl);
        Yii::$app->params['alternate'] = $main->Alternate($urlMonth, $mainUrl);

        $year = new Year();
        $yearData = $year->data($urlCheckData['year']);

        $calendar = new Calendar();
        $calendarByMonth = $calendar->byMonth($urlCheckData);
