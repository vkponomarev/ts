...
?><a name="calendar-<?= $dateData['year']['current'] ?>"></a><h1 class=main-page-h1><?= Yii::$app->params['text']['h1'] ?></h1><br><br><div class=row><?php /** Сегодняшний сезон */ ?><div class="col-xxs-12 col-xs-6 current-date"><div class=current-date-div><div class=current-date-one><div class=current-date-year><?= $dateData['year']['current'] ?><br><span class=current-date-month><?= Yii::t('app', 'year'); ?></span></div></div><div class=current-date-text><?= Yii::$app->params['text']['text1'] ?></div></div></div><?php /** Выберите страну */ ?><div class="col-xxs-12 col-xs-6 c-links-mp"><?php if ($dateData['year']['current'] >= 2000 && $dateData['year']['current'] <= 2030): ?><div class="c-links-block c-links-mp-header c-links-mp-header-link"><?= Yii::t('app', 'Choose the country'); ?></div><div class=c-links-block><form id=form><div class=form-group><select class=form-control name=country onchange='$("#form").submit()'><option><?= $countryData['name'] ?></option><?php foreach ($countriesData as $country) : ?><option value="<?= $country['id'] ?>"><?= $country['name'] ?></option><?php endforeach; ?></select></div></form></div><div class=c-links-block><hr></div><?php endif; ?><div class="c-links-block c-links-mp-header c-links-mp-header-link"><?= Yii::t('app', 'Seasons'); ?></div><div class=c-links-block><div class="col-xs-6 c-links-mp-months"><a href="/<?= Yii::$app->language ?>/calendar/seasons/winter/<?= $dateData['year']['current'] ?>/"><?= Yii::t('app', 'Winter') ?></a><br><a href="/<?= Yii::$app->language ?>/calendar/seasons/spring/<?= $dateData['year']['current'] ?>/"><?= Yii::t('app', 'Spring') ?></a><br></div><div class="col-xs-6 c-links-mp-months"><a href="/<?= Yii::$app->language ?>/calendar/seasons/summer/<?= $dateData['year']['current'] ?>/"><?= Yii::t('app', 'Summer') ?></a><br><a href="/<?= Yii::$app->language ?>/calendar/seasons/autumn/<?= $dateData['year']['current'] ?>/"><?= Yii::t('app', 'Autumn') ?></a><br></div></div><div class=c-links-block><hr class=hr-1></div></div></div><br><br><hr><?php
/**
 * Верхняя плашка календаря с годами туда сюда
 */

?><div class=row><div class="col-xxs-12 col-xs-4 c-prev-next-left"><a href="/calendar/years/<?= $dateData['year']['previous'] ?>/<?= (($getParamsByCalendarYears['countryGet'])
                && ($dateData['year']['previous'] >= $holidaysRange['start'] && $dateData['year']['previous'] <= $holidaysRange['end']))
                ? '?country=' . $countryData['id'] : '' ?>"><?= $dateData['year']['previous'] ?></a></div><div class="col-xxs-12 col-xs-4 c-prev-next-center"><?= $dateData['year']['current'] ?></div><div class="col-xxs-12 col-xs-4 c-prev-next-right"><a href="/calendar/years/<?= $dateData['year']['next'] ?>/<?= (($getParamsByCalendarYears['countryGet'])
                && ($dateData['year']['next'] >= $holidaysRange['start'] && $dateData['year']['next'] <= $holidaysRange['end']))
                ? '?country=' . $countryData['id'] : '' ?>"><?= $dateData['year']['next'] ?></a></div></div><hr><?php if ($PDFCalendarsData['exists']): ?><div class=row><div class="col-xxs-12 c-prev-next-right"><a href="#download-calendar-<?= $dateData['year']['current'] ?>"class="btn btn-default"><span class="fa fa-lg fa-print"></span>&nbsp;<?= Yii::t('app', 'Print'); ?></a></div></div><br><?php endif; ?><?php
/**
 ...
 */
?><?php if ($PDFCalendarsData['exists']): ?><br><?php if ($holidaysData): ?><a name="download-calendar-<?= $dateData['year']['current'] ?>"></a><h2 class=main-page-h1><?= Yii::t('app', 'Download and print PDF calendar with holidays and weekends for {year} {country_for}', [
                'country_for' => $countryData['name_for'],
                'year' => $dateData['year']['current'],
            ]) ?></h2><?php else: ?><a name="download-calendar-<?= $dateData['year']['current'] ?>"></a><h2 class=main-page-h1><?= Yii::t('app', 'Download and print PDF calendar for {year}', [
                'year' => $dateData['year']['current'],
            ]) ?></h2><?php endif; ?><br><br><div class="row rflex"><?php foreach ($PDFCalendarsData['pdf'] as $key => $pdf): ?><?php if ($pdf['pdfExists']): ?><div class="col-xxs-12 col-xs-6 col-md-4 col-lg-3 d-center pdf-download"><?php if ($key == 'P'): ?><div class=pdf-header><?= Yii::t('app', 'Holidays portrait calendar') ?></div><?php endif; ?><?php if ($key == 'L'): ?><div class=pdf-header><?= Yii::t('app', 'Holidays landscape calendar') ?></div><?php endif; ?><?php if ($key == 'PNoHolidays'): ?><div class=pdf-header><?= Yii::t('app', 'Portrait calendar without holidays') ?></div><?php endif; ?><?php if ($key == 'LNoHolidays'): ?><div class=pdf-header><?= Yii::t('app', 'Landscape calendar without holidays') ?></div><?php endif; ?><img alt=""class=c-download-img src="<?= $pdf['imgPathRelative'] ?>"width=100%> <a href="<?= $pdf['pdfPathRelative'] ?>"class="btn btn-success c-download-button"target=_blank download><?= Yii::t('app', 'Download') ?></a><br><a href="<?= $pdf['pdfPathRelative'] ?>"class="btn btn-success c-download-button"target=_blank><?= Yii::t('app', 'Print') ?></a></div><?php endif; ?><?php endforeach; ?></div><?php else: ?><?php endif; ?>