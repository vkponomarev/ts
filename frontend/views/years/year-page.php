            ...
            <div class="c-links-block">
                <form method="get" id="form">
                    <div class="form-group">
                        <select name="country" class="form-control" onchange="$('#form').submit();">
...
<?php if ($PDFCalendarsData['exists']): ?>
    ...
    <?php if ($holidaysData): ?>
        ...
        <h2 class="main-page-h1">
            <?= Yii::t('app', 'Download and print the calendar with holidays and weekends for {year} {country_for}', [
                'country_for' => $countryData['name_for'],
                'year' => $dateData['year']['current'],
            ]) ?>
        </h2>
    <?php else: ?>
        <a name="download-calendar-<?= $dateData['year']['current'] ?>"></a>
        <h2 class="main-page-h1">
            <?= Yii::t('app', 'Download and print the calendar for {year}', [
                'year' => $dateData['year']['current'],
            ]) ?>
        </h2>
    <?php endif; ?>
    ...
    <div class="row">
        <?php foreach ($PDFCalendarsData['pdf'] as $key => $pdf): ?>
            ...
            <div class="col-xxs-12 col-xs-6 col-md-4 d-center">
                ...
                <a href="<?= $pdf['pdfPathRelative'] ?>" download class="btn btn-success c-download-button">
                    <?= Yii::t('app', 'Скачать календарь на 2020 год') ?>
                </a>
                <a href="<?= $pdf['pdfPathRelative'] ?>" class="btn btn-success c-download-button">
                    <?= Yii::t('app', 'Распечатать календарь на 2020 год') ?>
                </a>
