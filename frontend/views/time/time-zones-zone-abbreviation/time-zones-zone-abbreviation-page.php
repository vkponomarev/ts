...
<div class="row">
    <div class="col-xs-12 plate-digital-watch">
        <span id="time" class="time-block"><?= $time->timezone->abbreviation->date->format('H:i:s') ?></span>
    </div>
</div>
<div class="row">
    <div class="col-xs-12 plate-digital-watch-text">
        ...
        <a href="/<?= Yii::$app->language ?>/calendar/years/<?= $date->year->current ?>/">
            <?= Yii::$app->formatter->asDate($time->timezone->abbreviation->date->format('Y-m-d H:i:s'), 'long') ?>
        </a>
        <?= ', ' ?>
        ...
        <a href="/<?= Yii::$app->language ?>/time/timezones/abbr/utc-317/<?= $time->timezone->abbreviation->offsetSimple ?>/">
            UTC <?= ' ' . $time->timezone->abbreviation->offset ?>
        </a>
    </div>
</div>
...
<?php if (isset($time->timezone->abbreviation->ianas->ianaData)) : ?>
    <br><br>
    <?php foreach ($time->timezone->abbreviation->ianas->ianaData as $key => $iana): ?>
        <?php if (isset($iana['cities'])) : ?>
            <a href="/<?= Yii::$app->language ?>/time/timezones/iana/<?= $iana['url'] ?>" class="main-header-a">
                <?= $iana['timezone_id'] ?>
            </a>
            <hr>
            <div class="row rflex">
                <?php foreach ($iana['cities'] as $city): ?>
                    <div class="col-xxs-12 col-xs-6 col-sm-4 col-md-3 plates">
                        <div class="plate-clock-min">
                            ...
                            <div class="plate-clock-min-digital-watch">
                                <span id="timeTimezonesCity<?= $key ?>"><?= $city['date']->format('H:i:s') ?></span>
                            </div>
                            <script type="text/javascript">
                                setInterval(
                                    function () {
