...
?><a name=time></a><h1 class=main-page-h1><?= Yii::$app->params['text']['h1'] ?></h1><hr><?php if ($time->geoIP->active) : ?><?= $this->render('../time-partials/_geoIP-row.min.php', [
    ...
    ?><?php endif; ?><?php if ($time->location->cities->byCountry->data) : ?><div class="rflex row"><?php foreach ($time->location->cities->byCountry->data as $key => $city): ?><div class="col-md-3 col-sm-4 col-xs-6 col-xxs-12 plates"><div class=plate-clock-min><div class=plate-clock-min-city-name><a href="/<?= Yii::$app->language ?>/time/cities/<?= $city['url'] ?>/"><?= $city['name'] ?></a></div><div class=plate-clock-min-digital-watch><span id="timeSecond<?= $key ?>"><?= $city['date']->format('H:i:s') ?></span></div><script>setInterval(
                        ...
                        , 1000);</script></div></div><?php endforeach ?></div><?php endif; ?>