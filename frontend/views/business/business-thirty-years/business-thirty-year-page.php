                ...
                <div class="plate-links">
                    <form method="get" id="form">
                        <div class="form-group">
                            <script>
                                let url = '<?php echo \yii\helpers\Url::home(true) . Yii::$app->language . '/calendar/business/thirty/years/' . $dateData['year']['current'] . '/';?>';
                            </script>
...
<div class="row">
    <div class="col-xxs-12 col-xs-4 c-prev-next-left">
        ...
        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/business/thirty/years/<?= $dateData['year']['previous'] ?>/
            <?= (($countryURL['url'] <> '')
    ...
    <div class="col-xxs-12 col-xs-4 c-prev-next-right">
        ...
        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/business/thirty/years/<?= $dateData['year']['next'] ?>/
            <?= (($countryURL['url'] <> '')
    ...
    foreach ($calendarByYear['calendar'] as $months) : ?>
        ...
        <div class="month col-xxs-12 col-xs-6 col-sm-4 col-md-3">
            ...
            <span class="fa fa-calendar">
                ...
                <a href="/<?= Yii::$app->language ?>/calendar/business/thirty/months/<?= $dateData['year']['current'] ?>-<?= str_pad($countMonths, 2, '0', STR_PAD_LEFT) ?>/
                    <?= (($countryURL['url'] <> '')
                    && ($dateData['year']['next'] >= $holidaysRange['start'] && $dateData['year']['next'] <= $holidaysRange['end']))
                    ? $countryURL['url'] . '/' : '' ?>">
...
<?php if ($holidaysData): ?>
    ...
    <h2 class="main-page-h1">
        <?= Yii::t('app', 'Calendar for the 36 hour work week for {year} {country_for}', [
...
<?php if ($PDFCalendarsData['exists']): ?>
    ...
    <h2 class="main-page-h1">
        <?= Yii::t('app', 'Download and print PDF calendar for the 36 hour work week for {year} {country_for}', [
            'country_for' => $countryData['name_for'],
            'year' => $dateData['year']['current'],
        ]) ?>
    </h2>
