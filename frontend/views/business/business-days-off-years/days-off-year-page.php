                ...
                <div class="plate-links">
                    <form method="get" id="form">
                        <div class="form-group">
                            <script>
                                let url = '<?php echo \yii\helpers\Url::home(true) . Yii::$app->language . '/calendar/days-off/years/' . $dateData['year']['current'] . '/';?>';
                            </script>
...
<div class="row">
    <div class="col-xxs-12 col-xs-4 c-prev-next-left">
        ...
        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/days-off/years/<?= $dateData['year']['previous'] ?>/
            <?= (($countryURL['url'] <> '')
    ...
    <div class="col-xxs-12 col-xs-4 c-prev-next-right">
        ...
        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/days-off/years/<?= $dateData['year']['next'] ?>/
            <?= (($countryURL['url'] <> '')
    ...
    foreach ($calendarByYear['calendar'] as $months) : ?>
        ...
        <div class="month col-xxs-12 col-xs-6 col-sm-4 col-md-3">
            ...
            <span class="fa fa-calendar">
                ...
                <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-<?= str_pad($countMonths, 2, '0', STR_PAD_LEFT) ?>/
                    <?= (($countryURL['url'] <> '')
                    && ($dateData['year']['next'] >= $holidaysRange['start'] && $dateData['year']['next'] <= $holidaysRange['end']))
                    ? $countryURL['url'] . '/' : '' ?>">
