        ...
        <div class="plate">
            ...
            <div class="plate-links">
                <div class="col-xs-6 plate-links-col ">
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-01/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[1] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-02/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[2] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-03/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[3] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-04/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[4] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-05/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[5] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-06/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[6] ?></a><br>
                </div>
                <div class="col-xs-6 plate-links-col">
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-07/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[7] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-08/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[8] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-09/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[9] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-10/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[10] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-11/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[11] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $dateData['year']['current'] ?>-12/
                ...
                <div class="plate-links">
                    <form method="get" id="form">
                        <div class="form-group">
                            <script>
                                let url = '<?php echo \yii\helpers\Url::home(true) . Yii::$app->language . '/calendar/business/quarters/' . $dateData['year']['current'] . '/' . $quarterURL .  '/';?>';
                            </script>
...
<div class="row">
    <div class="col-xxs-12 col-xs-4 c-prev-next-left">
        ...
        <?php if ($dateData['year']['previous'] < $holidaysRange['start']):?>

        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/business/quarters/<?= $dateData['year']['previous'] ?>/<?= $quarterURL ?>/
            <?= (($countryURL['url'] <> '')
    ...
    <div class="col-xxs-12 col-xs-4 c-prev-next-right">
        ...
        <?php else: ?>
            <a href="/<?= Yii::$app->language ?>/calendar/business/quarters/<?= $dateData['year']['next'] ?>/<?= $quarterURL ?>/
            <?= (($countryURL['url'] <> '')
...
<div class="row rflex year">
    <?php
    $countMonths = $calendarByQuarters['calendarStartMonth']-1;
    $countWeeks = 0;
    foreach ($calendarByQuarters['calendar'] as $months) :?>
        ...
        <div class="month col-xxs-12 col-xs-6 col-sm-4 col-md-3">
            ...
            <span class="fa fa-calendar">
                ...
                <a class="c-month-name" href="/<?= Yii::$app->language ?>/calendar/business/months/<?= $yearLink ?>-<?= str_pad($countMonths, 2, '0', STR_PAD_LEFT) ?>/
                    <?= (($countryURL['url'] <> '')
                    && ($dateData['year']['next'] >= $holidaysRange['start'] && $dateData['year']['next'] <= $holidaysRange['end']))
                    ? $countryURL['url'] . '/' : '' ?>">
            ...
            <table class="business-table">
                <tr class="business-gray">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['daysInMonth'][$countMonths] ?>
                    </td>
                </tr>
                <tr class="business-light">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['workingDays'][$countMonths] ?>
                    </td>
                </tr>
                <tr class="business-gray">
                    <td class="business-left">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['allHolidays'][$countMonths] ?>
                    </td>
                </tr>
                <tr class="business-ligh">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['hours40'][$countMonths] ?>
                    </td>
                </tr>
                <tr class="business-gray">
                    <td class="business-left">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['hours36'][$countMonths] ?>
                    </td>
                </tr>
                <tr class="business-ligh">
                    <td class="business-left">
                    ...
                    <td class="business-right">
                        <?= $calendarByQuarters['hours24'][$countMonths] ?>
                    </td>
                </tr>
            </table>
            <br>
...
<?php if ($holidaysData): ?>
    ...
    <h2 class="main-page-h1">
        <?= Yii::t('app', 'Business days calendar with holidays and weekends for the {quarter} quarter of {year} {country_for}', [
