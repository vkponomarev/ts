        ...
        <div class="plate">
            <div class="plate-header">
                <?php if ($countryURL['url'] <> '') : ?>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/years/<?= $dateData['year']['current'] ?>/
            ...
            <div class="plate-links">
                <div class="col-xs-6 plate-links-col ">
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-01/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[1] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-02/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[2] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-03/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[3] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-04/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[4] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-05/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[5] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-06/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[6] ?></a><br>
                </div>
                <div class="col-xs-6 plate-links-col">
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-07/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[7] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-08/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[8] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-09/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[9] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-10/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[10] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-11/
                <?= ($countryURL['url'] <> '') ? $countryURL['url'] . '/' : ''; ?>"><?= $calendarNameOfMonths[11] ?></a><br>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-12/
                ...
                <div class="plate-links">
                    <form method="get" id="form">
                        <div class="form-group">
                            <script>
                                let url = '<?php echo \yii\helpers\Url::home(true) . Yii::$app->language . '/calendar/days-off/months/' . $dateData['year']['current'] . '-' . $dateData['month']['number'] . '/';?>';
                            </script>
        ...
        <?php else: ?>
            ...
            <?php else: ?>
                <?php if ($dateData['month']['numberSimple'] == 1): ?>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['previous'] ?>-<?= str_pad(12, 2, '0', STR_PAD_LEFT) ?>/
            <?= (($countryURL['url'] <> '')
                ...
                <?php else: ?>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['current'] ?>-<?= str_pad($dateData['month']['numberSimple'] - 1, 2, '0', STR_PAD_LEFT) ?>/
            <?= (($countryURL['url'] <> '')
        ...
        <?php else: ?>
            ...
            <?php else: ?>
                <?php if ($dateData['month']['numberSimple'] == 12): ?>
                    <a href="/<?= Yii::$app->language ?>/calendar/days-off/months/<?= $dateData['year']['next'] ?>-<?= str_pad(1, 2, '0', STR_PAD_LEFT) ?>/
            <?= (($countryURL['url'] <> '')
...
<?php if ($holidaysData): ?>
    ...
    <h2 class="main-page-h1">
        <?= Yii::t('app', 'Days off calendar for {month} {year} {country_for}', [
...
<?php if ($PDFCalendarsData['exists']): ?>
    ...
    <h2 class="main-page-h1">
        <?= Yii::t('app', 'Download and print PDF days off calendar for {month} {year} {country_for}', [
            'country_for' => $countryData['name_for'],
            'year' => $dateData['year']['current'],
            'month' => $calendarNameOfMonths[$dateData['month']['numberSimple']],
        ]) ?>
            ...
            <?php if ($pdf['pdfExists']): ?>
                <div class="col-xxs-12 col-xs-6 col-md-4 col-lg-3 d-center pdf-download">
                    <?php if ($key == 'businessYearlyP'): ?>
                        <div class="pdf-header"><?= Yii::t('app', 'Portrait days off calendar for the year') ?></div>
                    <?php endif; ?>
                    <?php if ($key == 'businessYearlyL'): ?>
                        <div class="pdf-header"><?= Yii::t('app', 'Landscape days off calendar for the year') ?></div>
                    <?php endif; ?>
