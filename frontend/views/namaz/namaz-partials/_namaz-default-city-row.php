...
<div class="row">
    ...
    <div class="col-xxs-12 col-xs-4 col-lg-2 plate-namaz-times">
        <div class="plate-namaz-times-item-time">
            <?= $namaz->defaultCity->times['Sunrise'] ?>
        </div>
        <div class="plate-namaz-times-item">
            <?= Yii::t('app', 'Sunrise') ?>
        </div>
    ...
    <div class="col-xxs-12 col-xs-4 col-lg-2 plate-namaz-times">
        <div class="plate-namaz-times-item-time">
            <?= $namaz->defaultCity->times['Maghrib'] ?>
        </div>
        <div class="plate-namaz-times-item">
            <?= Yii::t('app', 'Maghrib') ?>
        </div>
...
<div class="row">
    <div class="col-xs-12 plate-namaz-times-text">
        ...
        <div class="plate-namaz-digital-watch">
            <a href="/<?= Yii::$app->language ?>/time/cities/<?= $time->location->defaultCity->url ?>/">
            <span id="timeOne" class="time-block">
                    <?= $time->location->defaultCity->date->format('H:i:s') ?>
            </span>
            </a>
            <?=  ', ' ?>
        </div>
        ...
        &nbsp;
        <a href="/<?= Yii::$app->language ?>/time/countries/<?= $time->location->defaultCountry->url ?>/">
            <?= $time->location->defaultCountry->name ?></a><?= ', ' ?>
        <a href="/<?= Yii::$app->language ?>/namaz/cities/<?= $time->location->defaultCity->url ?>/">
            <?= $time->location->defaultCity->name ?>
        </a>
        <?= ', ' ?>
        <?= $date->day->name . ', ' ?>
        <a href="/<?= Yii::$app->language ?>/calendar/years/<?= $date->year->current ?>/">
            <?= Yii::$app->formatter->asDate($time->location->defaultCity->date->format('Y-m-d H:i:s'), 'long') ?>
        </a>
        <?= ', ' ?>
        ...
        <?= ', ' ?>
        <a href="/<?= Yii::$app->language ?>/time/timezones/abbr/utc-317/<?= $time->location->defaultCity->offsetSimple ?>/">
            UTC <?= ' ' . $time->location->defaultCity->offset ?>
        </a>
    </div>
</div>
<br>
