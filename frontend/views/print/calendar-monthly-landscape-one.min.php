...
?><table class=calendar-pdf-header-table><tr><td class=calendar-pdf-header-table-left><?php if (isset($getParamsCustomize['header']) && $getParamsCustomize['header'] <> ''): ?><h1 class=calendar-pdf-header-h1><?= $getParamsCustomize['header'] ?></h1><h2 class=calendar-pdf-header-h2><?= Yii::t('app', 'Calendar for {month} {year}. ({country})', [
                        ...
                        'country' => $countryData['name'],
                        'month' => $calendarNameOfMonths[$dateData['month']['numberSimple']]
                    ]) ?></h1><?php endif; ?><td class=calendar-pdf-header-table-right><h3 class=calendar-pdf-header-h3><a href=https://timesles.com>TIMESLES.COM</a></h3></table><br><table class=cpdf-ml1-year><?php
    $countMonths = $dateData['month']['numberSimple']-1;
    $countWeeks = 0;
    foreach ($calendarByMonths as $months) :?><?php $countMonths++; ?><tr><td class=cpdf-ml1-td-month><table class=cpdf-ml1-month><tr class=cpdf-ml1-week-day-name><?php for ($i = 1; $i <= 7; $i++): ?><td class=cpdf-ml1-day-name><?= $calendarNameOfDaysInWeek[$i]; ?></td><?php endfor; ?><tr><td class=cpdf-ml1-week-line colspan=7></tr><?php foreach ($months as $week): ?><tr class=cpdf-ml1-week><?php for ($i = 1; $i <= 5; $i++): ?><?php if (isset($week[$i])): ?><?php $key = array_search($week[$i]['date'], array_column($holidaysData, 'date'));
                                    if (false !== $key): ?><td class="cpdf-ml1-day cpdf-ml1-day-holiday"><?= $week[$i]['monthDay']; ?></td><?php else: ?><td class="cpdf-ml1-day cpdf-ml1-day"><?= $week[$i]['monthDay']; ?></td><?php endif; ?><?php else: ?><td class="cpdf-ml1-day cpdf-ml1-no-day"><span></span></td><?php endif; ?><?php endfor; ?><?php for ($i = 6; $i <= 7; $i++): ?><?php if (isset($week[$i])): ?><?php $key = array_search($week[$i]['date'], array_column($holidaysData, 'date'));
                    ...
                    for ($i = $count; $i < $countHolidaysHalf; $i++) :
                        ...
                        ?><tr><td class=cpdf-ml1-holidays-name><?= Yii::$app->formatter->asDate($dateFormat, 'medium'); ?><td class=cpdf-ml1-holidays-rounded-mark><td class=cpdf-ml1-holidays-name><?= $holidaysData[$i]['name']; ?></tr><?php endfor; ?></table><td><table class=cpdf-ml1-holidays-table-inside><?php
                    $count = $countHolidaysHalf;

                    for ($i = $count; $i < $countHolidays; $i++) :
                        $dateFormat = new \DateTime($holidaysData[$i]['date']);
                        ?><tr><td class=cpdf-ml1-holidays-name><?= Yii::$app->formatter->asDate($dateFormat, 'medium'); ?><td class=cpdf-ml1-holidays-rounded-mark><td class=cpdf-ml1-holidays-name><?= $holidaysData[$i]['name']; ?></tr><?php endfor; ?></table></table></div>