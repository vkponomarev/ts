...
';
                if (($countLimit >= 49998) or ($songsLinksCount == $count)) {
...
' . $siteMapUrls . '
                    ...
                    // Создаем файл
                    $siteMap = new SiteMap();
                    $fileName = 'sitemap_songs_ru_' . $fileNameAdd . '_' . $countFiles;
                    (new \common\components\dump\Dump())->printR($siteMap->realPath());
                    $siteMap->generateFile($siteMapContent, $fileName . '.xml',  $siteMap->realPath() . 'gii/sitemap/ru/songs/');
