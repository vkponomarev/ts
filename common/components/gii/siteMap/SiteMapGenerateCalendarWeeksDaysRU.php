...
class SiteMapGenerateCalendarWeeksDaysRU
{
    ...
    function generate($languagesData)
    {
        ...
        foreach ($languagesData as $language) {
            ...
            $siteMapUrls .= \Yii::$app->view->render('@common/components/gii/siteMap/templates/_calendar-weeks-days.php', [
                'language' => $language,
                'date' => 'today',
            ]);
            ...
            while ($eachDay->format('Y-m-d') <= $endDayFormatted) {
                ...
                if (($countLimit >= 49998) or (($eachDay->format('Y-m-d') == $endDayFormatted))) {
                    ...
                    // Создаем файл
                    $gii = new Gii();
                    $fileName = 'sitemap_calendar_weeks_days_ru_' . $countFiles;
