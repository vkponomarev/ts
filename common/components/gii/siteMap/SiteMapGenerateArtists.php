...
';
                if (($countLimit >= 49998) or (($artistsLinksCount == $count) and ($languagesDataCount == $countLang))) {
...
' . $siteMapUrls . '
                    ...
                    // Создаем файл
                    $siteMap = new SiteMap();
                    $fileName = 'sitemap_artists_' . $countFiles;
                    (new \common\components\dump\Dump())->printR($siteMap->realPath());
                    $siteMap->generateFile($siteMapContent, $fileName . '.xml',  $siteMap->realPath() . 'gii/sitemap/artists/');
